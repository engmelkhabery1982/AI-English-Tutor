 ...
 */
export function stepDownSupport(current: ShadowingSupportLevel): ShadowingSupportLevel {
  if (current === 'full_transcript') return 'partial_transcript';
  if (current === 'partial_transcript') return 'audio_only';
  return 'audio_only';
}

/**
 * Pure deterministic policy for shadowing support progression.
 *
 * Rules:
 * - 'insufficient_evidence' => no support reduction
 * - 'different' => retain current support
 * - 'close' => conservative progression only after sufficient repeated evidence (>= 2 at current level)
 * - 'matched' => may reduce by ONE support step (never skips levels)
 * - never skip multiple support levels at once
 * - replay does not change support
 * - failed STT does not change support
 */
export function computeShadowingSupport(
  base: ShadowingSupportLevel,
  history: readonly (ShadowingQualitativeResult | { readonly qualitative: ShadowingQualitativeResult })[],
): ShadowingSupportLevel {
  let current = base;
  let closeCountAtCurrentLevel = 0;

  for (const item of history) {
    const qualitative = typeof item === 'string' ? item : item.qualitative;
    if (qualitative === 'matched') {
      current = stepDownSupport(current);
      closeCountAtCurrentLevel = 0;
    } else if (qualitative === 'close') {
      closeCountAtCurrentLevel += 1;
      if (closeCountAtCurrentLevel >= 2) {
        current = stepDownSupport(current);
        closeCountAtCurrentLevel = 0;
      }
    } else if (qualitative === 'different') {
      closeCountAtCurrentLevel = 0;
    } else if (qualitative === 'insufficient_evidence') {
      // Retain current support
    }
  }
...
/**
 * Deterministic attempt-count support mapping (deprecated helper).
 * computeShadowingSupport should be preferred for earned progression.
 */
 ...
 *
 * OWNERSHIP:
 * 1. Shadowing content match is deterministic transcript-vs-target (matched | close | different | insufficient_evidence).
 *    This OWNS the final qualitative category.
 * 2. Existing PronunciationEngine only ADDS qualitative pronunciation observations.
 *    It must NEVER upgrade or downgrade lexical target matching.
 *    - wrong words + clear pronunciation => different
 *    - close words + clear pronunciation => close
 *    - exact words + unclear pronunciation => content matched, plus pronunciation warning
 *    - pronunciation unavailable => local match remains
 */
export async function runShadowingAttempt(
  ...
  try {
    ...
    const pronLines = outcome.feedbackLines.filter((line) => line.trim().length > 0);
    const feedbackLines = [...local.feedbackLines];
    if (outcome.analysis.overallIntelligibility === 'unclear') {
      const hasWarning = pronLines.some((line) => /unclear|clarity|pronunciation/i.test(line));
      if (!hasWarning) {
        feedbackLines.push('Pronunciation was unclear — try to articulate each word clearly.');
      }
    }
    for (const line of pronLines) {
    ...
    return {
      transcript,
      qualitative: local.qualitative,
      feedbackLines,
      evaluatedBy: 'pronunciation',
      judged: local.judged,
    };
  } catch {
...
export class ShadowingSession {
  ...
  private readonly attemptHistory: ShadowingAttempt[] = [];
  ...
  get history(): readonly ShadowingAttempt[] {
    return this.attemptHistory;
  }

  /** The support in effect for the NEXT attempt. */
  get support(): ShadowingSupportLevel {
    return computeShadowingSupport(this.baseSupport, this.attemptHistory);
  }
  ...
  async submit(
    ...
    if (outcome.judged) {
      this.attempts += 1;
      this.attemptHistory.push(outcome);
    }
...
/**
 * Asynchronously resolves real voice infrastructure (recorder + STT)
 * using the app's existing providers if not injected.
 */
export async function resolveVoiceInput(
  ...
  try {
    ...
    if (!resolvedStt) {
      ...
      resolvedStt = talkDemo.createGeminiSTTProvider({ apiKey });
    }
    const resolvedRecorder = recorder ?? talkDemo.createExpoAudioRecorder();
    return { recorder: resolvedRecorder, stt: resolvedStt };
  } catch {
    return null;
  }
}
...
export class ShadowingVoiceController {
  ...
  async stopAndJudge(): Promise<ShadowingAttempt | ShadowingVoiceFailure> {
    ...
    let judged: ShadowingAttempt;
    try {
      if (this.submitOverride) {
        judged = await this.submitOverride(transcript);
      } else {
        judged = await this.session.submit(transcript, this.port, this.now);
      }
    } catch {
      return this.failure('transcription-failed', SHADOWING_TRANSCRIPTION_FAILED_MESSAGE);
    }
